...
export function TestimonialMarquee({ testimonials, speed = 50 }: TestimonialMarqueeProps) {
    ...
    const TestimonialCard = ({ testimonial }: { testimonial: Testimonial }) => (
        <Card className="flex-shrink-0 w-[400px] mx-3 bg-white border-zinc-200 hover:border-zinc-300 hover:shadow-md transition-all">
            <CardContent className="p-6 flex flex-col h-full">
                <Quote className="h-8 w-8 text-blue-500/20 mb-4" />

                <p className="text-zinc-600 text-base leading-relaxed flex-1 mb-6">
                    "{testimonial.content}"
                </p>

                <div className="flex items-center gap-4 pt-4 border-t border-zinc-200">
                    <div className="w-12 h-12 rounded-full bg-zinc-100 flex items-center justify-center overflow-hidden flex-shrink-0">
                    ...
                    <div className="flex-1 min-w-0">
                        <div className="font-medium text-zinc-900 truncate">{testimonial.name}</div>
